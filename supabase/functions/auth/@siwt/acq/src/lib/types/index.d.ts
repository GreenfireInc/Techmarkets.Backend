...
export type GetOwnedAssetsForPKH = ({ network, contract, pkh, contractType, }: {
    network: Network;
    contract: string;
    pkh: string;
    contractType: AssetContractType;
}) => Promise<BigmapKeyResponse[]>;
export type GetBalance = ({ network, contract }: {
    network: Network;
    contract: string;
}) => Promise<number>;
export type GetTokenBalance = ({ network, contract, pkh, tokenId, }: {
    network: Network;
    contract: string;
    pkh: string;
    tokenId: string;
}) => Promise<number>;
export type GetAttributesFromStorage = ({ network, contract, tokenId, }: {
    network: Network;
    contract: string;
    tokenId: string;
}) => Promise<BigmapKeyResponse[]>;
export type GetAssetContractTypeByContract = ({ contract, network, }: {
    contract: string;
    network: Network;
